...
public class RivalTree extends StateNode implements TreeInterface {
	    ...
	    final public Input<String> nodeTypeInput = new Input<>("nodetype",
	            "type of the nodes in the beast.tree", RivalNode.class.getName());
	     ...
	     */
	    protected RivalNode root;
	    protected RivalNode storedRoot;

	    /**
	     * array of all nodes in the tree *
	     */
	    protected RivalNode[] m_nodes = null;

	    protected RivalNode[] m_storedNodes = null;
...
//	            leafNodeCount = other.leafNodeCount;
	    ...
	    public void makeCaterpillar(final double minInternalHeight, final double step, final boolean finalize) {
	        ...
	        RivalNode left = new RivalNode();
	        left.labelNr = 0;
	        left.height = 0;
	        left.setID(taxa.get(0));
	        for (int i = 1; i < taxa.size(); i++) {
	            final RivalNode right = newRivalNode();
	            right.labelNr = i;
	            right.height = 0;
	            right.setID(taxa.get(i));
	            final RivalNode parent = newRivalNode();
	    ...
	    protected void processTraits(List<TraitSet> traitList) {
	        for (TraitSet traitSet : traitList) {
	            for (RivalNode node : getExternalNodes()) {
	     ...
	     */
	    protected RivalNode newNode() {
	        try {
	            return (RivalNode) Class.forName(nodeTypeInput.get()).newInstance();
	        } catch (Exception e) {
	    ...
	    protected void initArrays() {
	        ...
	        m_nodes = new RivalNode[nodeCount];
	        listNodes(root, m_nodes);
	        m_storedNodes = new RivalNode[nodeCount];
	        final RivalNode copy = root.copy();
	    ...
	    public Tree(final RivalNode rootNode) {
	        setRoot(rootNode);
	        initArrays();
	    }
	    ...
	    protected void adjustTreeNodeHeights(final RivalNode node) {
	        if (!node.isLeaf()) {
	            for (final RivalNode child : node.getChildren()) {
	     ...
	     */
	    @Override
		public List<RivalNode> getExternalNodes() {
	        final ArrayList<RivalNode> externalNodes = new ArrayList<>();
	        for (int i = 0; i < getNodeCount(); i++) {
	            final RivalNode node = getNode(i);
	     ...
	     */
	    @Override
		public List<RivalNode> getInternalNodes() {
	        final ArrayList<RivalNode> internalNodes = new ArrayList<>();
	        for (int i = 0; i < getNodeCount(); i++) {
	            final RivalNode node = getNode(i);
	     ...
	     */
	    public void setRootOnly(final RivalNode root) {
	    ...
	    @Override
		public RivalNode getNode(final int nodeNr) {
	    ...
	    void collectTaxaNames(final RivalNode node) {
	        if (node.getID() != null) {
	        ...
	        if (node.isLeaf()) {
	        	...
	        	for (RivalNode child : node.getChildren()) {
	     ...
	     */
	    @Override
		public void getMetaData(final RivalNode node, final Double[] t, final String pattern) {
	     ...
	     * @param pattern the name of the meta data
	     */
	    public void getMetaData(final RivalNode node, final Integer[] t, final String pattern) {
	     ...
	     */
	    @Override
		public void setMetaData(final RivalNode node, final Double[] t, final String pattern) {
	     ...
	     */
	    void listNodes(final RivalNode node, final RivalNode[] nodes) {
...
//	    }
	    ...
	    protected RivalNode[] postCache = null;
	    @Override
		public RivalNode[] listNodesPostOrder(RivalNode node, RivalNode[] nodes) {
	        if( node != null ) {
	     ...
	     */
	    @Override
		public RivalNode[] getNodesAsArray() {
	        return m_nodes;
	    }
	    ...
	    public void assignTo(final StateNode other) {
	        ...
	        final RivalNode[] nodes = new RivalNode[nodeCount];
	    ...
	    public void assignFrom(final StateNode other) {
	        final Tree tree = (Tree) other;
	        final RivalNode[] nodes = new RivalNode[tree.getNodeCount()];//tree.getNodesAsArray();
	        for (int i = 0; i < tree.getNodeCount(); i++) {
	            nodes[i] = newRivalNode();
	        }
	        setID(tree.getID());
	        //index = tree.index;
	    ...
	    public void assignFromFragile(final StateNode other) {
	        ...
	        final RivalNode[] otherNodes = tree.m_nodes;
	     ...
	     */
	    private void assignFrom(final int start, final int end, final RivalNode[] otherNodes) {
	        for (int i = start; i < end; i++) {
	            RivalNode sink = m_nodes[i];
	            RivalNode src = otherNodes[i];
...
//	    }
	     ...
	     */
	    public static void printTranslate(final RivalNode node, final PrintStream out, final int nodeCount) {
	     ...
	     */
	    static void printTranslate(RivalNode node, List<String> translateLines, int nodeCount) {
	        if (node.isLeaf()) {
	    ...
	    public static void printTaxa(final RivalNode node, final PrintStream out, final int nodeCount) {
	        final List<String> translateLines = new ArrayList<>();
	        printTranslate(node, translateLines, nodeCount);
	        Collections.sort(translateLines);
	    ...
	    protected void store() {
	        ...
	        if (m_storedNodes.length != nodeCount) {
	            final RivalNode[] tmp = new RivalNode[nodeCount];
	    ...
	    private void storeNodes(final int start, final int end) {
	        ...
	        for (int i = start; i < end; i++) {
	            final RivalNode sink = m_storedNodes[i];
	            final RivalNode src = m_nodes[i];
	            sink.height = src.height;
	            ...
	            final List<RivalNode> children = sink.children;
	            final List<RivalNode> srcChildren = src.children;

	            if( children.size() == srcChildren.size() ) {
	               // shave some more time by avoiding list clear and add
	               for (int k = 0; k < children.size(); ++k) {
	                   final RivalNode srcChild = srcChildren.get(k);
	                   // don't call addChild, which calls  setParent(..., true);
	                   final RivalNode c = m_storedNodes[srcChild.getNr()];
	                   c.parent = sink;
	                   children.set(k, c);
	               }
	            } else {
	                ...
	                for (final RivalNode srcChild : srcChildren) {
	                    // don't call addChild, which calls  setParent(..., true);
	                    final RivalNode c = m_storedNodes[srcChild.getNr()];
	                    c.parent = sink;
	                    children.add(c);
	    ...
	    public void restore() {
	        ...
	        final RivalNode[] tmp = m_storedNodes;
	     ...
	     */
	    public String getTaxonId(final RivalNode node) {
	    ...
	    public void removeNode(final int i) {
	        final RivalNode[] tmp = new RivalNode[nodeCount - 1];
	     ...
	     * Use with care!
	     */
	    public void addNode(final RivalNode newNode) {
	        final RivalNode[] tmp = new RivalNode[nodeCount + 1];
